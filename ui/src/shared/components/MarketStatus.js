...
const MarketStatus = () => {
  ...
  const formatStatus = (marketStatus) => {
    switch (marketStatus) {
      case 'premarket':
        return <Box sx={{color: theme.palette.success.light, display: 'inline', fontWeight: 600}}>Pre Market</Box>
      case 'open':
        return <Box sx={{color: theme.palette.success.main, display: 'inline', fontWeight: 600}}>Open</Box>
      case 'postmarket':
        return <Box sx={{color: theme.palette.success.dark, display: 'inline', fontWeight: 600}}>Post Market</Box>
      case 'closed':
        return <Box sx={{color: theme.palette.warning.main, display: 'inline', fontWeight: 600}}>Closed</Box>
      default:
        return marketStatus
    }
  }
  ...
  return (
    <Container className='MarketStatus'>
      <Box sx={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', mx: '24px', my: '8px'}}>
